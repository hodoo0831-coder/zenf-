...
### 한글 줄바꿈 — 조판이 허술해 보이는 첫째 원인

브라우저 기본값은 한글을 **낱글자 단위로** 끊는다. "자동검증"이 "자동검 / 증"으로,
"현장관리자"가 "현장관리 / 자"로 갈라진다. 좁은 칸이 많은 관리 화면에서는 이게 화면
전체를 아마추어처럼 보이게 만든다. body에 한 줄이면 끝난다.

```css
body{ word-break:keep-all; overflow-wrap:break-word }
```

`keep-all`이 어절을 지키고, `overflow-wrap`이 그래도 안 들어가는 긴 토큰(URL·사번)을
넘치지 않게 받아준다. 둘을 같이 써야 한다.

`keep-all`도 **하이픈 뒤는 끊는다**(`V-01~15` → `V- / 01~15`). 코드·규칙번호처럼 붙어
있어야 뜻이 통하는 토큰은 **논브레이킹 하이픈 U+2011(`‑`)** 로 쓴다. 보이는 모양은
같고 끊기지만 않는다.
...
**통계 타일은 개수에 맞는 열 수로 배치한다.** `auto-fit`에 맡기면 8개가 6+2로 흘러
오른쪽 네 칸이 비고, 그 순간 "설계가 없다"는 인상을 준다. 렌더 후 `fitTiles()`가 개수를
보고 `--tc`를 넣는다(3~5 중 딱 떨어지는 가장 큰 값, 없으면 4). 8개는 4×2, 6개는 3×2로
떨어진다. 1280 미만은 폭이 우선이라 `auto-fit` 그대로 둔다.
...
- 화면 골격(상단바·컨텍스트바·사이드바·탭바·드로어), 카드, 통계 타일, 표·리스트 카드,
  진행 단계, 판정 행(점검·검수용), 모달, 토스트, 배지·태그, 날짜축 그리드, 달력,
  로그인 화면이 들어 있다.
...
"좀 더 전문적으로"라는 말이 나오면 대개 아래 중 하나가 걸린 것이다.

- **컬러 이모지.** 📤📄🔒 같은 것이 버튼·메뉴에 있으면 헤어라인 흑백 화면에 풀컬러
  그림이 섞여 소비자 앱처럼 보인다. 아이콘이 정말 필요하면 글자색을 따라오는 기하 기호
  (`✓ ✕ → ★ ▤`)를 쓰고, 아니면 **빼는 쪽이 낫다.** 관리 화면의 메뉴는 글자만으로 충분하다.
- **의미 없는 색.** 강조가 네이비인데 hover나 본문 강조만 초록으로 남아 있는 식. 색이
  뜻을 잃으면 상태 배지도 같이 안 읽힌다.
- **깨진 격자.** 위 타일 배치 문제. 마지막 줄만 덩그러니 비는 것.
- **낱글자 줄바꿈.** 위 `word-break:keep-all`.

넷 다 장식을 더해서가 아니라 **걷어내서** 해결된다. 이 톤에서 무언가 부족해 보이면
대개 더 넣을 것이 아니라 뺄 것이 남은 것이다.
