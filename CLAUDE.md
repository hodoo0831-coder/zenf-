...
- UI 텍스트는 한국어, 브랜딩은 젠키퍼(ZenKeeper)·제니엘.
  강조색은 네이비(`--navy:#0f2438`), 바탕은 흰색(`--bg:#ffffff`),
  초록(`--green:#5b7f52`)은 정상·성공을 뜻하는 자리에만 쓴다.
- 화면 디자인은 `.claude/skills/zen-ui` 스킬을 따른다 — 팔레트·글꼴(나눔고딕 내장)·
  비율 규칙과 반응형 검증 스크립트가 들어 있다.
